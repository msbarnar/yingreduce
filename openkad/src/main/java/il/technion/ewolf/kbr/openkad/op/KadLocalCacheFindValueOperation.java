...
public class KadLocalCacheFindValueOperation extends FindValueOperation
  ...
  KadLocalCacheFindValueOperation(@Named("openkad.local.node") final Node localNode,
                                  ...
                                  @Named("openkad.cache.share") final int nrShare,
                                  final Provider<FindNodeRequest> findNodeRequestProvider,
                                  final Provider<MessageDispatcher<Node>> msgDispatcherProvider,
                                  final KBuckets kBuckets,
                                  final Provider<StoreMessage> storeMessageProvider,
                                  final Communicator kadServer,
                                  final KadCache cache,

                                  @Named(
                                      "openkad.testing.nrLocalCacheHits") final AtomicInteger nrLocalCacheHits,
                                  @Named(
                                      "openkad.testing.nrRemoteCacheHits") final AtomicInteger nrRemoteCacheHits) {
    ...
    this.alreadyQueried = new HashSet<>();
    this.querying = new HashSet<>();

    this.lastSentTo = new LinkedList<>();
  ...
  private boolean trySendFindNode(final Node to) {
    ...
    FindNodeRequest
        findNodeRequest =
        this.findNodeRequestProvider.get().setSearchCache(true).setKey(this.key);

    return this.msgDispatcherProvider.get().addFilter(new IdMessageFilter(findNodeRequest.getId()))
        .addFilter(new TypeMessageFilter(FindNodeResponse.class)).setConsumable(true)
        .setCallback(to, this)
  ...
  private void sortKnownClosestNodes() {
    this.knownClosestNodes =
        sort(this.knownClosestNodes, on(Node.class).getKey(), this.keyComparator);
