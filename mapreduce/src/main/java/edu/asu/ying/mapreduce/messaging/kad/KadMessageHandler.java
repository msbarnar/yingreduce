...
/**
 * {@link KadMessageHandler} dispatches messages from the Kademlia network by firing a {@link FilteredValueEvent},
 * allowing listeners to be notified of specific messages which they select by attaching a filter to the event.
 * </p>
 * Each {@link KadMessageHandler} listens for messages on a specific scheme.</br>
 * E.g. if the message handler is bound to the scheme {@code resource}, it will receive messages with URIs like:
 * </p>
 * {@code resource\host:port\type\name}
 * </p>
 * The event can be injected anywhere using the annotation {@link IncomingMessageEvent}.
 */
	...
	implements MessageHandler, il.technion.ewolf.kbr.MessageHandler
{
	 ...
	 */
	public final void bind(final String scheme) {
		this.inputNode.register(scheme, this);
	}
	/**
	 * Provides a {@link FilteredValueEvent} that provides incoming messages to other listeners.
	 * </p>
	 * Any class can receive messages of a specific type by attaching a filter and callback to this event.
	 */
	@Override
	...
	/**
	 * Relays an incoming message via the {@link FilteredValueEvent} and returns an
