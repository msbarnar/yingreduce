...
/**
 * When fired, {@code FilteredValueEvent} only notifies event handlers whose {@link Filter} matches the value fired.
 */
	...
	implements FilteredValueEvent<TValue>
{
	...
	/**
	 * Notifies event handlers whose filter matches {@code value} that value has arrived.
	 * </p>
	 * If a handler returns {@code false} from its {@link EventHandler#onEvent} method, the handler and its filter
	 * are detached from this event.
