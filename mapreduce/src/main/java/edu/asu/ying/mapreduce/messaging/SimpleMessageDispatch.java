...
/**
 * {@link SimpleMessageDispatch} is a {@link edu.asu.ying.mapreduce.io.MessageOutputStream} that associates messages with specific recipient
	...
	implements MessageDispatch
{
	...
	public final FutureMessage getFutureMessage() {
		...
		// Fix for now on making the default match match nothing; the race condition then produces false negatives
	...
	public FutureMessage getFutureMessage(final MessageFilter filter) {
		// Solves the race condition in getFutureMessage() on specifying the match before adding the message
		synchronized (this.futures) {
			final FutureMessage future = this.createFutureMessage();
